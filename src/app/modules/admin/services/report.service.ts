...
export class ReportService implements OnInit {
  ...
  findWeeksByCourseId(courseId: string) {
    ...
    return this.afs.collection(COLLECTION_NAME_WEEKS, ref => ref.where('course', '==', courseId).orderBy('timestamp', 'asc')).get();
  }
  ...
  findDataReportByUidPlanificacion(uidPlanifation: string) {
    ...
    return this.afs.collection(COLLECTION_NAME_REPORT, ref => ref.where('uidPlanification', '==', uidPlanifation)).get();
...
*/
  async updateDetailsPlanificationReporte(reporIde: string, newItem: any) {
    ...
    try {
      ...
      const planiDocRef = this.afs.collection(COLLECTION_NAME_REPORT).doc(reporIde);
...
* @returns 
*/
  async updateDetailsPlanificationStatusCountUploadReport(reporIde: string, newItem: any) {
    ...
    try {
      ...
      await this.afs.firestore.runTransaction(async (transaction) => {
        ...
        const index = items.findIndex((item: any) => item.uid_teacher == newItem.itemReport.uid_teacher);
        ...
        console.log("transaction updateDetailsPlanificationStatusCountUploadReport", items);
...
async updateDetailsPlanificationStatusReport(reporIde: string, data: any) {
  ...
  try {
    ...
    await this.afs.firestore.runTransaction(async (transaction) => {
      ...
      // buscar el index  donde el uid_teacher sea igual al uid del teacher que subio el reporte
      const index = items.findIndex((item: any) => item.uid_teacher == data.uid_teacher);
